...
namespace Utils.SerializeInterface.Editor {
    ...
    public class RequireInterfaceDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            /*
             * When a field attribute like RequireInterfaceAttribute is applied to an array field, the OnGUI method is called once for the entire array.
             * This is different from a property drawer that targets a specific type, which is called for each array element (see InterfaceReferenceDrawer)
             */
        ...
        /// <summary>
        /// Determines the most appropriate base type for a Unity ObjectField, based on the field's type
        /// and a required interface.
        /// </summary>
        /// <remarks>
        /// This method acts as a heuristic to provide the <see cref="EditorGUI.ObjectField"/>
        /// with a valid type. Since an ObjectField cannot directly display an interface type,
        /// this method returns a concrete type (like <see cref="MonoBehaviour"/> or <see cref="ScriptableObject"/>)
        /// that objects implementing the interface are likely to derive from.
        /// <br/>
        /// The returned type limits the objects that can be dragged into the inspector field,
        /// while the final validation check ensures the assigned object properly implements the required interface.
        /// </remarks>
        /// <param name="fieldType">The type of the serialized field, which can be a single object, an array, or a List.</param>
        /// <param name="interfaceType">The required interface type for the field.</param>
        /// <returns>
        /// The most specific base <see cref="Type"/> to use for the ObjectField, or <see cref="Object"/>
        /// as a final fallback.
        /// </returns>
