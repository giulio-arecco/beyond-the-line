...
Davanti a {HasCompanion("Elias") or HasCompanion("Lira"):voi|te}, prima che la strada si restringa verso il vero e proprio valico, sorge una struttura tozza in cemento armato. Un vecchio avamposto per i minatori, o forse un punto di controllo dismesso.
La porta è scardinata, ma è l'unico riparo visibile prima dell'abisso.

{ HasCompanion("Elias"):
    Elias si stringe nel cappotto, i denti che battono con un suono ritmico, incontrollabile. Guarda l'edificio con sospetto, ma il freddo è un nemico più immediato di qualsiasi cosa possa nascondersi lì dentro.
...
=== miners_outpost ===
Varcate la soglia dell'edificio.
L'interno è in penombra e l'aria è ferma, pesante, carica d'odore di muffa e grasso per motori. <nl>
La stanza principale è devastata. Una parte del soffitto in calcestruzzo ha ceduto, creando una barriera di travi marce, calcinacci e tondini di ferro contorti che taglia in due l'ambiente.
...
{ KNOWN_MOUNTAINPASS_STATUS:
    Se {HasCompanion("Elias") or HasCompanion("Lira"):avete|hai} intuito correttamente il significato della comunicazione via radio alla casa di comando, la via mineraria potrebbe essere satura di gas. Quelle casse potrebbero contenere l'equipaggiamento che potrebbe fare la differenza tra la vita e la morte.
- else:
    Osservi le casse. Sono di fattura militare recente, troppo nuove per questo posto abbandonato. Qualcuno le ha stoccate qui prima che il soffitto crollasse. Se contengono provviste o equipaggiamento, potrebbero fare la differenza tra la vita e la morte.
}
...
+ {HasCompanion("Elias") or HasCompanion("Lira")} [Chiedi aiuto per spostare le macerie.]
    "Ho bisogno di una mano" dici, indicando la trave più grossa.
    { HasCompanion("Elias"):
...
=== loot_mask ===
Ti insinui nel varco e raggiungi le casse. <nl>
Con il cuore che rimbomba nel petto, forzi il coperchio della prima.
...
{ HasCompanion("Elias") or HasCompanion("Lira"):
    Ti giri verso {HasCompanion("Elias"):Elias|Lira} con la maschera in mano.
...
+ [Ti fermi per analizzare la struttura.]
    ...
    ++ [Non hai modo di liberare il cavo. Bisogna scegliere un'altra via.]
        Il ghiaccio è troppo duro per romperlo a mani nude {HasItem("Pistol"):o col calcio della pistola}.
        Ti rialzi, frustrato.
        -> bridge_crossing_choices
...
+ [Attraversi di corsa, saltando le assi marce.]
    ...
    Non ti fidi a restare appeso lì sopra troppo a lungo. Il freddo vi sta uccidendo stando fermi e la struttura geme sotto il vento.
    ...
    {HasCompanion("Lira"):Siete|Sei} a metà strada quando una raffica di vento improvvisa investe la gola.
...
=== elias_accident ===
{ crossing_method == crawl:
    Tocca a Elias. È terrorizzato. Si muove a scatti, abbracciando la trave.
...
{ crossing_method == cable:
    Elias avanza tenendosi al cavo. Improvvisamente, mette un piede su una chiazza di ghiaccio nero invisibile sulla trave.
...
{ crossing_method == run:
    Senti un rumore lacerante alle tue spalle. Elias, troppo pesante e scoordinato per quel ritmo, atterra male su un'asse marcia che cede all'istante.
    Lui perde l'equilibrio e scompare nel buco, riuscendo ad aggrapparsi a una traversina con una sola mano un attimo prima di precipitare.
...
+ [Ti lanci per afferrarlo]
    ...
    Ti butti a terra, sporgendoti nel vuoto. Afferri il polso di Elias.
    ...
    { GetGlobalStat("Fatigue") > fatigue_threshold:
        ...
        I {HasCompanion("Lira"):vostri sguardi si incrociano|suoi occhi incrociano i tuoi}. Terrore puro.
        La presa si spezza.
        Elias cade nel buio.
...
+ [È troppo pericoloso. Resti indietro.]
    Il rischio è troppo alto. Il ponte sta per crollare anche sotto di te.
    Indietreggi, guardandolo con orrore. "Mi dispiace" sussurri.
    
    Elias cerca di resistere, ma le forze lo abbandonano. Le dita si aprono.
    Scompare nel vuoto con un urlo strozzato.
...
Non c'è vento qui. Il gas, più pesante dell'aria, ha saturato la parte bassa del tunnel, coprendo i binari con una marea giallastra, densa come latte andato a male, che vortica pigramente intorno agli stivali.
...
{ HasCompanion("Elias") and not elias_fell:
    Elias indietreggia bruscamente, coprendosi la bocca. "Vesk!" esclama, la voce strozzata dal terrore. "Vesk-gora!"
    Conosce questo odore. Sa cosa fa ai polmoni.
}

{ HasCompanion("Lira"):
    Lira osserva la passerella in alto, poi indica una grata divelta e contorta sulla parete. "Il condotto di ventilazione è crollato" constata freddamente. "Non c'è via d'uscita lassù. Dobbiamo passare dal tunnel principale."
...
{ HasCompanion("Elias") and not elias_fell:
    Elias guarda la scala di servizio e scuote la testa. È ferito, esausto e corpulento.
...
+ {HasItem("GasMask") && HasItem("Crowbar") && (HasCompanion("Elias") or HasCompanion("Lira"))} [Divisione Strategica: Squadre separate.]
    ...
    { HasCompanion("Elias") and not elias_fell:
        ...
        L'aria lassù è respirabile, ma satura di umidità ferrosa. Sotto di voi, Elias avanza nella nebbia gialla.
        
        Avanzate paralleli. Improvvisamente, la passerella è sbarrata da una paratia di sicurezza in ferro, saldata dalla ruggine. Non c'è modo di aprirla da lì.
        "Il meccanismo è giù!" urli a Elias attraverso il frastuono del sangue nelle orecchie e il ronzio delle lampade rotte.
        
        Elias non vede nulla. La maschera è appannata, il gas è denso e avvolgente.
        "Vex?" grida, la voce ovattata e metallica.
        Devi guidarlo a voce.
        
        ++ ["A sinistra! Dietro i detriti!"]
            Elias annaspa, sposta macerie alla cieca. Senti la passerella scricchiolare pericolosamente sotto il tuo peso mentre il metallo stride.
            "Drov! Hek!"
    ...
    - else:
        ...
        Sei immerso nel gas. È come camminare dentro un liquido sporco. La visibilità è zero, il mondo è ridotto a ombre giallastre.
...
+ {HasItem("GasMask") && (HasCompanion("Elias") or HasCompanion("Lira"))} [Condividete la maschera sui binari.]
    ...
    "Restiamo uniti" dici. "Respiri contati. Scambio rapido."
    ...
    A metà strada, tocca a te passare la maschera a {HasCompanion("Elias"):Elias|Lira}.
    Le tue mani sono sudate, tremano per la paura e l'adrenalina.
    ...
    Ti getti a terra per recuperarla, mentre ti lacrimano gli occhi per l'esposizione al gas.
    La maschera si è incastrata sotto un vecchio carrello minerario, con la cinghia di gomma che sporge oltre le ruote.
    Elias è in apnea totale. Inizia a tossire, gli occhi sbarrati dal panico, le vene del collo che pulsano mentre cerca disperatamente aria che non sia veleno.
    
    ++ [Afferri la cinghia e tiri con forza]
        Non c'è tempo per la delicatezza. Tiri con violenza brutale.
        ...
        La premi sul volto sudato di Elias. La maschera ora è danneggiata, non terrà più perfettamente, ma lui respira, emettendo un suono rauco e disperato.
        Correte verso l'uscita.
        
        -> convergence
        
    ++ [Ti fermi a districare il nodo con calma.]
        "Fermo!" ordini, bloccando le sue mani che annaspano.
        Costringi le tue dita a muoversi con precisione chirurgica mentre il gas ti brucia gli occhi facendoli lacrimare copiosamente.
        Ci vogliono cinque secondi. Cinque secondi eterni in cui il tuo compagno inala veleno, sussultando.
...
+ {not HasItem("GasMask") and not HasItem("Crowbar") and (HasCompanion("Elias") or HasCompanion("Lira"))} [Correte attraverso il gas.]
    ~ crossing_method = run
    Nessun metodo per evitare il gas. Nessun sostegno per raggiungere la passerella. Non c'è altra scelta che correre in apnea.
    "Dobbiamo correre" dici, con voce tremante. "Trattenete il fiato. Non fermatevi."
    
    { HasCompanion("Elias"): Elias annuisce, terrore puro negli occhi. }
    { HasCompanion("Lira"): Lira stringe i pugni fino a sbiancare le nocche: "Andiamo." }

    Vi lanciate nel tunnel, immergendovi nella nube tossica.
    
    Dopo pochi secondi, il gas penetra ovunque. Il sapore di mandorle amare è sulla lingua, insopportabile.
    I polmoni implorano aria, ma inspirare significa morire.
    ...
    ++ [Ti volti e torni indietro alla cieca.]
        Non puoi lasciarli. Ti giri nel fumo, annaspando con le braccia.
        ...
        { GetGlobalStat("Fatigue") >= 100:
            Mentre vi avvicinate alla luce, la tua vista inizia ad offuscarsi, prima ai lati, poi completamente. I suoni iniziano a rimbobarti come scoppi nelle orecchie, accompagnati dal ritmo del battito cardiaco.
    ...
    ++ [Segui la rotaia e urli di seguirti.]
        Non puoi fermarti o morirete tutti. Ti butti a terra, seguendo il metallo freddo della rotaia.
        "Seguite la mia voce!" gridi, sputando saliva e cararro.
        Procedi carponi, veloce come un animale in trappola. Senti dei passi trascinati dietro di te, ma non ti volti.
...
+ {not HasCompanion("Elias") and not HasCompanion("Lira") and HasItem("GasMask")} [Avanzi nel tunnel con la maschera.]
    ...
    A metà strada, il percorso è sbarrato. Una paratia di sicurezza è scesa sui binari, bloccando il passaggio.
    Sei solo. Nessuno sulla passerella in alto può azionare il contrappeso per sollevarla. Nessuno può dirti se c'è un'altra via.
    Devi trovare il meccanismo manuale qui in basso, nel buio e nel gas.
    
    Tasti la paratia e trovi una ruota idraulica incrostata di ruggine e sale minerale.
    Provi a girarla. È bloccata.
    ...
    ++ [Usi la forza bruta per sbloccare la ruota.]
        Punti i piedi nel fango tossico. Afferri la ruota con entrambe le mani e tiri, urlando nella maschera.
    ...
    ++ {HasItem("Crowbar")} [Usi il piede di porco come leva.]
        Inserisci il piede di porco tra i raggi della ruota. Fai leva con tutto il peso del corpo.
        Il metallo si piega, ma la ruota gira. La paratia si alza quel tanto che basta.
        Ti getti oltre con il cuore che batte all'impazzata.
        ~ IncreaseGlobalStat("Fatigue", 10)
        { GetGlobalStat("Fatigue") >= 100:
            Non appena ti trovi dall'altro lato, ti si offusca la vista, ti cedono le gambe e crolli nel fango. 
            Sei sfinito. Lo sforzo fisico degli ultimi giorni è stato insopportabile e ora il tuo corpo non ce la fa più. 
            -> collapse_physical
        }
        -> convergence
...
+ {not HasCompanion("Elias") and not HasCompanion("Lira") and not HasItem("GasMask")} [Corri attraverso il gas.]
    ~ crossing_method = run
    Puoi solo contare sui tuoi polmoni.
    Prendi una boccata d'aria e ti lanci nel tunnel, immergendoti nella nube tossica.
    
    Dopo trenta secondi sembra che i polmoni abbiano preso fuoco. Lacrimi finché il mondo non diventa una macchia indistinta.
    ...
    ++ [Ti fidi dei tuoi occhi e corri verso le "luci".]
        Vedi un chiarore. Sembra l'uscita. Corri verso di essa.
        Sbatti violentemente contro la parete di roccia. Era un'allucinazione.
        ...
        { GetGlobalStat("Fatigue") >= 100:
            Cerchi di rialzarti, ma la vista inizia ad offuscarsi, prima ai lati, poi completamente. I suoni iniziano a rimbobarti come scoppi nelle orecchie, accompagnati dal ritmo del battito cardiaco.
            Il gas inalato, la carenza di ossigeno e la fatica devastante degli ultimi giorni stanno colpendo nel momento peggiore.
            -> collapse_hypoxia
        }
        
        Ti rialzi per puro istinto di sopravvivenza, trascinandoti alla cieca.
        Arrivi all'uscita per miracolo, strisciando come un animale ferito.
        -> convergence
        
    ++ [Chiudi gli occhi e segui la rotaia al tatto.]
        Ignori i lampi di luce finta. Ti butti a terra, faccia nel fango chimico.
        La rotaia è fredda, umida, solida. È l'unica cosa reale in questo incubo.
        La segui con la mano scorticata, usandola come unica indicazione nella nube di gas.
        ...
        { GetGlobalStat("Fatigue") >= 100:
            Mentre ti avvicini a quella che sembra la luce dell'uscita, la tua vista inizia ad offuscarsi, prima ai lati, poi completamente. I suoni iniziano a rimbobarti come scoppi nelle orecchie, accompagnati dal ritmo del battito cardiaco.
            Il gas inalato, la carenza di ossigeno e la fatica devastante degli ultimi giorni stanno colpendo nel momento peggiore. Inciampi nelle tue stesse gambe, crollando al suolo.
            -> collapse_hypoxia
        }
...
{ HasCompanion("Elias") or HasCompanion("Lira"):
    ~ DecreaseGlobalStat("Cohesion", 20)
    Senti delle mani che ti afferrano, ti scuotono. Voci preoccupate che appaiono infinitamente lontane.
    ...
    Ti trascinano per gli ultimi metri. Sei un peso morto, un fardello.
...
- else:
    Svieni e rinvieni diverse volte nel giro di minuti che sembrano eterni. Ogni volta che cerchi di rimetterti in piedi crolli di nuovo a terra, finchè non collassi sbattendo la testa e perdi completamente i sensi.
    Il risveglio è lento e doloroso. Potresti essere svenuto per secondi o minuti preziosi.
...
=== collapse_hypoxia ===
La gola ti brucia come se fosse piena di tizzoni ardenti.
Le gambe non rispondono più.
Luce... casa? Uscita?
Cadi sul fianco.
Cos'era? Un ratto? Impossibile... il gas...
Quello è il soffitto o il pavimento...?

{ HasCompanion("Elias") or HasCompanion("Lira"):
    ~ DecreaseGlobalStat("Cohesion", 20)
    Qualcuno ti sta trascinando. La roccia gratta la pelle.
    "R-respira! Dannazione!"
    Uno schiaffo. Aria gelida nei polmoni. Un colpo di tosse che ti strappa il torace.
    Apri gli occhi. Lira... o Elias... ti guardano con terrore. Ti hanno dovuto portare fuori di peso. Hai rischiato di uccidere tutti rallentandoli.
    Ti senti stordito, inutile.
- else:
    Nulla.
    ...
    ...
    Riemergi come da un annegamento profondo. Il primo respiro è uno spasmo che ti piega in due. I polmoni fischiano, rifiutano l'aria fredda.
    Sei disteso poco oltre lo sbocco dei tunnel. Devi aver strisciato per puro istinto mentre la mente era spenta.
...
La discesa verso valle è più dolce qui. La nebbia si dirada a tratti, lasciando intravedere la piana sottostante.
Linee di trincee. Filo spinato. Crateri.
La terra di nessuno.

{ elias_fell:
    Improvvisamente, senti un rumore di sassi che rotolano alla tua destra, dove il canalone risale per incontrare il sentiero.
    Una mano guantata, tremante, afferra il bordo della strada. Poi un'altra.
    
    Elias si issa sulla strada, rotolando sulla schiena. È una maschera di fango ghiacciato, il cappotto lacerato in più punti, le mani sanguinanti. Il suo petto si alza e si abbassa in spasmi violenti.
    Ha scalato la parete del canalone, esposto al vento e al gelo, per aggirare il tunnel. È vivo, ma è l'ombra di se stesso.
...
Poi, noti qualcosa.
Poco sotto la linea del sentiero, incastrata in un anfratto di roccia al riparo dal vento dominante, c'è una piccola capanna di pietra.
Un filo di fumo azzurrognolo esce da un buco nel tetto.
...
{ HasCompanion("Lira"):
    Lira estrae la pistola, controllando il caricatore. "Potrebbe essere un posto di osservazione avanzato, o un cecchino" sussurra.
}
{ HasCompanion("Elias"):
    Elias fissa il fumo con desiderio disperato. Trema così forte che i suoi denti fanno un rumore udibile.
}

{HasCompanion("Elias") or HasCompanion("Lira"):Vi avvicinate|Ti avvicini} silenziosamente. Non ci sono fili d'allarme, né impronte di scarponi militari recenti. Solo tracce di legna trascinata.
Sulla soglia della capanna, seduto su una cassetta di munizioni vuota, c'è un uomo.
È anziano, con una barba ispida e vestiti fatti di pelli e stracci. Pulisce un vecchio fucile da caccia con gesti lenti e metodici.

Non {HasCompanion("Elias") or HasCompanion("Lira"):vi ha ancora visti|ti ha ancora visto}. Oppure finge di non averlo fatto.
...
* {HasItem("Pistol")} [Avanzi armi in pugno. Minacci l'uomo per prendere il rifugio.]
    ...
    Il vecchio non sussulta. Alza lo sguardo lentamente, posa il fucile a terra e alza le mani aperte. I suoi occhi sono pietre grigie, privi di paura, pieni solo di un'infinita stanchezza.
...
* [Ti avvicini a mani alzate. Chiedi ospitalità.]
    ...
    {HasCompanion("Elias") or HasCompanion("Lira"):Vi|Ti} fa entrare. La capanna è piccola, vieni abbracciato dal calore del fuoco.
    Mette un pentolino sul fuoco. Zuppa di radici e carne secca.
    { HasCompanion("Elias"):
        Quando porge la ciotola a Elias, vede i resti della divisa nemica sotto il cappotto. Si ferma un secondo. Elias si irrigidisce.
...
* ["Dobbiamo attraversare il fronte."]
    "Il fronte," ripete. Sembra assaggiare la parola e trovarla stantia. "Una linea immaginaria tracciata da uomini che bevono vino in stanze riscaldate. Ho visto quella linea spostarsi avanti e indietro per vent'anni. Prima erano gli uomini del re, poi i rivoluzionari, poi la mia gente, ora la tua"
    "La terra non riconosce le uniformi. Seppellisce tutti con la stessa indifferenza."
    -> hermit_first_dialogue

* ["Perché vivi qui, isolato da tutto?"]
    Il vecchio alza lo sguardo. I suoi occhi sono lattiginosi, privi di giudizio.
    "Perché laggiù bisogna scegliere una bugia per cui morire".
    -> hermit_first_dialogue

* -> 
    Sputa nel fuoco. Le braci sfrigolano.
...
{ HasItem("Notebook") and not HasCompanion("Elias"):
    ...
    * [Gli chiedi se sa tradurre il taccuino]
        Lo tiri fuori. La pelle è fredda al tatto.
        "Sai leggere questo?" chiedi, porgendogli il taccuino. "Credo sia scritto nella tua lingua."
        
        Il vecchio lo prende con gesti lenti. Apre la prima pagina. Non c'è emozione sul suo volto, solo la noia di chi ha già letto quella storia mille volte.
        
        "Non sono ordini" dice secco. "È una lista della spesa di un condannato a morte".
        
        "Traduci".
        
        Lui legge, la voce monocorde, priva di pietà:
        "Mia piccola Sofie. Oggi la neve sembra lo zucchero della tua torta. Vorrei mandartela, ma si scioglierebbe. Mi manchi. Mi manca il pane di tua madre. Ho paura di dimenticare le vostre facce. Se torno, giuro che non toccherò mai più un fucile..."
        
        Chiude il taccuino di scatto e te lo lancia indietro.
        "Solo un uomo, che ora sarà cibo per vermi in un fosso dimendicato da dio. La neve coprirà anche lui."
        
        Guardi il taccuino. Ricordi vagamente l'uomo che l'ha scritto e i suoi occhi che incrociavano i tuoi mentre lasciavi la fattoria.
